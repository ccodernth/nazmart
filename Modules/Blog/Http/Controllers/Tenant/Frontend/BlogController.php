...
class BlogController extends Controller
{
    ...
    public function setLocaleTenantBlog($lang)
    {
        session(['lang' => $lang]);
    ...
    public function blog_single($slug)
    {
        ...
        $blog_post = Blog::with(['user', 'category', 'comments'])->where(['slug->' . $localeLang => $slug, 'status' => 1])->first();
        ...
        if (!$blog_post) {
            ...
            foreach ($languages as $language) {
                if (app()->getLocale() != $language->slug) {
                    $blog_post = Blog::with(['user', 'category', 'comments'])->where(['slug->' . $language->slug => $slug, 'status' => 1])->first();
                    if ($blog_post) {
                        $this->setLocaleTenantBlog($language->slug, $slug);
                        break;
                    }
                }
            }
    ...
    public function blog_search_page(Request $request)
    {
        ...
        $all_blogs = Blog::Where('title->' . app()->getLocale(), 'LIKE', '%' . $request->search . '%')
