            ...
            <ul class="pagination-list">
                @if(count($links) > 1)
                    @foreach($links as $key => $link)
                        <li>
                            <a data-page="{{ $key + 1 }}" href="{{ $link }}" class="page-number {{ ($key + 1) === $current_page ? 'current' : '' }}">
                                {{ $key + 1 }}
                            </a>
                        </li>
                    @endforeach
                                 ...
                                 @foreach($links as $link)
                                     <li>
                                         <a data-page="{{ $loop->iteration }}" href="{{ $link }}"
                                            class="page-number {{ $loop->iteration === $current_page ? "current" : ""}}">{{ $loop->iteration }}</a>
                                     </li>
                                 @endforeach
                             @endif
                         </ul>
                ...
                <div class="category-pagination-out mt-60">
                    <nav aria-label="Page navigation">
                        <ul class="pagination pagination-sm pagination-list">
                            @if(count($links) > 1 )
                                @foreach($links as $key => $link)
                                    <li class="page-item {{ $key + 1 == $current_page ? 'active' : '' }}">
                                        <a class="page-link" href="{{ $link }}">{{ $key + 1 }}</a>
                                    </li>
                                @endforeach
                            @endif
                        </ul>
