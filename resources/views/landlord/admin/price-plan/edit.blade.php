                    ...
                    <div class="tab-content">
                        @foreach(get_all_language() as $language)
                            ...
                            <div class="tab-pane fade show {{ $loop->iteration == 1 ? 'active' : '' }}"
                                ...
                                <x-fields.input name="translate[{{$language->slug}}][title]" label="{{__('Title')}}"
                                                value="{{$plan->getTranslations()['title'][$language->slug]}}"
                                />

                                <x-fields.input name="translate[{{$language->slug}}][package_badge]" label="{{__('Package Badge')}}"
                                                value="{{$plan->getTranslations()['package_badge'][$language->slug] ?? ''}}"
                                />
                                <x-fields.textarea name="translate[{{$language->slug}}][description]" label="{{__('Package Description')}}"
                                                   value="{{$plan->getTranslations()['description'][$language->slug] ?? ''}}"
                                />
                                <div class="iconbox-repeater-wrapper">
                                    @php
                                        //  dd($plan->faq);
                                              $faq_items = !empty($plan->getTranslations()['faq']) ? unserialize($plan->getTranslations()['faq'][$language->slug] ) : ['title' => ['']];
                                    @endphp
                                    @forelse($faq_items['title'] as $faq)
                                        <div class="all-field-wrap">
                                            <div class="form-group">
                                                <label for="faq">{{__('Faq Title')}}</label>
                                                <input type="text" name="translate[{{$language->slug}}][faq][title][]" class="form-control" value="{{$faq}}">
                                            </div>
                                            <div class="form-group">
                                                <label for="faq_desc">{{__('Faq Description')}}</label>
                                                <textarea name="translate[{{$language->slug}}][faq][description][]"
                                    ...
                                    @empty
                                        <div class="all-field-wrap">
                                            <div class="form-group">
                                                <label for="faq">{{__('Faq Title')}}</label>
                                                <input type="text" name="translate[{{$language->slug}}][faq][title][]" class="form-control"
                                                       placeholder="{{__('faq title')}}">
                                            </div>
                                            <div class="form-group">
                                                <label for="faq_desc">{{__('Faq Description')}}</label>
                                                <textarea name="translate[{{$language->slug}}][faq][description][]" class="form-control f_desc"
