...
class PaymentSettingsController extends Controller
{
    ...
    {
		$this->middleware('permission:payment-settings-kapital', ['only' => [
            'kapital_settings', 'update_payment_settings']]);
		$this->middleware('permission:payment-settings-payriff', ['only' => [
            'payriff_settings', 'update_payriff_settings']]);
	...
	public function payriff_settings()
    {
        $gateway = PaymentGateway::where('name', 'payriff')->first();
        return view(self::BASE_PATH . 'payment_settings', compact('gateway'));
    }
	
	public function kapital_settings()
    {
        $gateway = PaymentGateway::where('name', 'kapital')->first();
        return view(self::BASE_PATH . 'payment_settings', compact('gateway'));
    }
