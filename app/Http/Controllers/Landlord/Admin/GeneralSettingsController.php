...
class GeneralSettingsController extends Controller
{
    ...
    public function setAllDb()
    {
        try {
            ...
            /* Page */
            $pages = StaticOption::get();

            foreach ($pages as $page) {
                $pageUpdateData = StaticOption::query()
                    ->find($page->id);
                $designationList = [];

                foreach ($langList as $lang) {
                    $content = $page->option_value;
                    if (!is_array(json_decode($content, true))) {
                        $designationList[$lang->slug] = $page->option_value;
                    }
                }
                $pageUpdateData->option_value = $designationList;
    ...
    public function update_basic_settings(Request $request)
    {
        ...
        foreach ($fields as $field_name => $rules) {
            update_static_option($field_name, $translate[$field_name]);
        }
    ...
    public function update_seo_settings(Request $request)
    {
        ...
        $translate =$this->convertTranslateMetas($request->input('translate'));
        foreach ($fields as $field_name => $rules) {
            update_static_option($field_name, $translate[$field_name]);
        }
...
//            case "queue":
    ...
    public function setLocaleAdmin(Request $request, $param)
    {
