...
class PricePlanController extends Controller
{
    ...
    public function setAllDb()
    {
        try {
            ...
            foreach ($pages as $page) {
                $pageUpdateData = PricePlan::query()
                    ->find($page->id);
                $designationList = [];
                $descriptionList = [];
                ...
                foreach ($langList as $lang) {
                    ...
                    if (!is_array(json_decode($content, true))) {
                        $designationList[$lang->slug] = $page->faq;
                    }
                }
                $pageUpdateData->faq = $designationList;
    ...
    public function store_new_price_plan(Request $request)
    {
        ...
        try {
            ...
            $price_plan->title = $translate['title'];
            $price_plan->package_badge = $translate['package_badge'];
            $price_plan->description = $translate['description'];
            $price_plan->faq = $translate['faq'];
            if (!tenant()) {
    ...
    public function update(Request $request)
    {
        ...
        try {
            ...
            $price_plan->description = $translate['description'];
            $price_plan->faq = $translate['faq'];
            // dd($translate, $translate['faq'], $price_plan);
            if (!tenant()) {
            ...
            foreach ($value as $key2 => $item) {
                ...
                if ($key2 == 'faq') {
                    ...
                    if (!empty($faq_item)) {
                        ...
                        $result[$key2][$key] = serialize($faq_set);
                    }
                } else {

                    $result[$key2][$key] = SanitizeInput::esc_html($item ?? '');
                }
            }
